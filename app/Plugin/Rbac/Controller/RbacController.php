...
class RbacController extends AppController {
	...
	protected function _saveRow($data, $redirect = 'index'){
		...
		if($this->{$this->modelClass}->validates()){
			if($this->{$this->modelClass}->save($data)){
				$this->DatabaseMenus->clearCache();
				$this->_flash(__('Los cambios han sido guardados.', true), 'success');
				if($redirect != 'noredirect'){
					$this->redirect(array('action' => $redirect));
				}
			}else{
				$this->_flash(__('Los cambios no fueron guardados. Por favor, intente nuevamente.', true), 'error');
			}
		}else{
			$this->_flash(__('Los cambios no fueron guardados. Datos inválidos.', true), 'error');
		}
	}
	...
	protected function _checkIdPresence($id = null, $redirect = 'index'){
		if(!$id){
			$this->_flash(__('Petición invalida. Sin Identificación se ha proporcionado.', true), 'error');
			if($redirect != 'noredirect'){
				$this->redirect(array('action' => $redirect));
			}
		}
